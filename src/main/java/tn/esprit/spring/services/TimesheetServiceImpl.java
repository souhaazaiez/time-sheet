...
//
	...
	public long getNbTimesheet() {
		/*int i=0;
		timesheetRepository.findAll().forEach(t -> );
		return ((List<Timesheet>)timesheetRepository.findAll()).size();*/
		return timesheetRepository.count();
	}

	public Timesheet getTimesheet(TimesheetPK tpk) {
		return timesheetRepository.findBytimesheetPK(tpk);
	}

	public void deleteTimesheet(TimesheetPK tpk) {
		timesheetRepository.delete(getTimesheet(tpk));
	}
}
